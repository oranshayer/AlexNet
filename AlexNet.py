...
def distorted_inputs():
  ...
  images,labels = AlexNet_input.distorted_inputs(data_dir=FLAGS.data_dir,
...
def inputs(eval_data):
  ...
  images, labels = AlexNet_input.inputs(eval_data=eval_data,
                                        data_dir=data_dir,
                                        batch_size=FLAGS.batch_size)
  if FLAGS.use_fp16:
    images = tf.cast(images, tf.float16)
...
#  tf.image_summary('images', images, max_images=6)
  ...
  # norm1
  norm1 = tf.nn.lrn(conv1, 5, bias=2.0, alpha=0.0001, beta=0.75,
                    name='norm1')
  # pool1
  pool1 = tf.nn.max_pool(norm1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                         padding='SAME', name='pool1') # The padding method for maxpool must be 'VALID' for our needs
  ...
  # norm2
  norm2 = tf.nn.lrn(conv2, 5, bias=2.0, alpha=0.0001, beta=0.75,
                    name='norm2')
  # pool2
